...
용어 :
  - 랜더링 : 컴포넌트 호출하여 VDOM에 적용하는 과정
             전체과정 - 1. 컴포넌트 호출 return react element
                        2. VDOM 재조정 작업 (여기까지 랜더링)
                        3. render가 컴포넌트 정보를 DOM에 MOUNT
                        4. 브라우저가 DOM을 paint 

  - react element
    - 컴포넌트 호출 시 return 하는 것 ( react.createElement() 호출)
    - 컴포넌트의 정보(결국 DOM에 삽입될 내용: type, key, props, ref, children)
      를 담은 객체 

  - fiber
    - VDOM의 노드 객체
    - react elemebt 의 내용이 DOM에 반영되기 위해 , 먼저 VDOM에 추가되어야 하는데
...
1. react 코어
  -Component 정의
    ...
    - Hooks를 사용하여 함수형 콤포넌트에서도 state 와 side effects를 관리 할 수 있다.
